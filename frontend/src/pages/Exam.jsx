...
export default function Exam() {
  ...
  if (!report) return <p className="busy">Loading exam…</p>;
    ...
    <section className="exam">
      <div>
        <div className="stage">
          ...
          {overlayUrl && <img className="overlay" src={overlayUrl} alt={`${overlay} heatmap`} />}
          <div className="grid-hit" onClick={onClick} title="Click a region" />
        </div>
        <div className="toggles">
          {OVERLAYS.map((o) => (
            <button
              key={o.id}
              type="button"
              className={`chip ${overlay === o.id ? "on" : ""}`}
        ...
        {local && (
          <div className="local">
            Tile r{local.row + 1}, c{local.col + 1}: blur {local.tile.blur.toFixed(2)} · under{" "}
      ...
      <aside className="panel">
        ...
        <div className="issues">
          {(report.issues || []).length === 0 && <span className="muted">No issues above threshold.</span>}
          {(report.issues || []).map((iss) => (
            ...
            >
              ...
              <div className="muted">
                {iss.severity} · {(iss.confidence * 100).toFixed(0)}%
              </div>
            </button>
          ))}
        </div>
